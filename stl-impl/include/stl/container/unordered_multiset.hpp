#ifndef STL_UNORDERED_MULTISET_HPP
#define STL_UNORDERED_MULTISET_HPP
...
struct unordered_multiset_key_of_value {
    const Key& operator()(const Key& value) const {
        return value;
    }
};

// unordered_multiset容器实现
template <typename Key, typename Hash = stl::hash<Key>, typename KeyEqual = stl::equal_to<Key>, typename Allocator = stl::allocator<Key>>
...
public:
    ...
    explicit unordered_multiset(const allocator_type& alloc) : table_(0, hasher(), key_equal(), alloc) {}
    ...
    template <typename InputIterator>
    unordered_multiset(InputIterator first, InputIterator last, size_type bucket_count, const allocator_type& alloc)
        : table_(bucket_count, hasher(), key_equal(), alloc) {
        insert(first, last);
    }
    
    template <typename InputIterator>
    unordered_multiset(InputIterator first, InputIterator last, size_type bucket_count, const hasher& hash, const allocator_type& alloc)
    ...
    unordered_multiset(std::initializer_list<value_type> init, size_type bucket_count, const allocator_type& alloc)
        : table_(bucket_count, hasher(), key_equal(), alloc) {
        insert(init.begin(), init.end());
    }
    
    unordered_multiset(std::initializer_list<value_type> init, size_type bucket_count, const hasher& hash, const allocator_type& alloc)
        : table_(bucket_count, hash, key_equal(), alloc) {
        insert(init.begin(), init.end());
    }
    ...
    unordered_multiset& operator=(std::initializer_list<value_type> init) {
        clear();
        insert(init.begin(), init.end());
        return *this;
    }
...
private:
    // 继承哈希表并实现get_key方法
    class unordered_multiset_table : public table_type {
    public:
    ...
    unordered_multiset_table table_;
};

// unordered_multiset交换特化
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
void swap(unordered_multiset<Key, Hash, KeyEqual, Allocator>& a, 
         unordered_multiset<Key, Hash, KeyEqual, Allocator>& b) noexcept {
    a.swap(b);
}

} // namespace stl

#endif // STL_UNORDERED_MULTISET_HPP