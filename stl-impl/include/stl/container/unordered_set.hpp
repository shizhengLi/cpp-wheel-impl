#ifndef STL_UNORDERED_SET_HPP
#define STL_UNORDERED_SET_HPP
...
struct unordered_set_key_of_value {
...
public:
    ...
    explicit unordered_set(const allocator_type& alloc) : table_(0, hasher(), key_equal(), alloc) {}
    ...
    template <typename InputIterator>
    unordered_set(InputIterator first, InputIterator last, size_type bucket_count, const allocator_type& alloc)
        : table_(bucket_count, hasher(), key_equal(), alloc) {
        insert(first, last);
    }
    
    template <typename InputIterator>
    unordered_set(InputIterator first, InputIterator last, size_type bucket_count, const hasher& hash, const allocator_type& alloc)
    ...
    unordered_set(std::initializer_list<value_type> init, size_type bucket_count, const allocator_type& alloc)
        : table_(bucket_count, hasher(), key_equal(), alloc) {
        insert(init.begin(), init.end());
    }
    
    unordered_set(std::initializer_list<value_type> init, size_type bucket_count, const hasher& hash, const allocator_type& alloc)
        : table_(bucket_count, hash, key_equal(), alloc) {
        insert(init.begin(), init.end());
    }
...
private:
    // 继承哈希表并实现get_key方法
    class unordered_set_table : public table_type {
    public:
    ...
    unordered_set_table table_;
};

// unordered_set交换特化
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
void swap(unordered_set<Key, Hash, KeyEqual, Allocator>& a, 
         unordered_set<Key, Hash, KeyEqual, Allocator>& b) noexcept {
    a.swap(b);
}

} // namespace stl

#endif // STL_UNORDERED_SET_HPP